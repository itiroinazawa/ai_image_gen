...
def verify_device(logger, requested_device: str) -> torch.device:
    ...
    if requested_device == "mps" and not hasattr(torch.backends, "mps") and not getattr(torch.backends, "mps", {}).get("is_available", False):
        logger.warning("MPS (Apple Silicon) requested but not available. Falling back to CPU.")
        return torch.device("cpu")
    ...
    try:
        ...
        logger.warning(f"Error initializing device {requested_device}: {str(e)}. Falling back to CPU.")
        return torch.device("cpu")


def verify_precision(logger, requested_device: torch.device, requested_precision: str) -> str:
    """
    ...
    if requested_device.type == "cuda" and requested_precision == "fp16":
        try:
            ...
            logger.warning(f"FP16 precision not supported on this CUDA device: {str(e)}. Falling back to FP32.")
            return "fp32"
